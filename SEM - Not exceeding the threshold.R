...
df=scale(df[,1:15],center=TRUE,scale=TRUE)
df <- as.data.frame(df)
mymod <- psem(
  lm(SOC ~ FMD + GPP + PR + Tavg + N, data = df),
  lm(FMD ~ PR + Tavg + N + ELE, data = df),
  lm(GPP ~ Tavg + N + ELE, data = df),
  lm(PR ~ ELE, data = df),
  lm(Tavg ~ ELE, data = df),
  data = df)
